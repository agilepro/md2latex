...
/**
 * Maps Markdown info strings onto language names the {@code listings} package
 * actually knows.
 *
 * <p>This matters more than it looks. Passing {@code language=json} to
 * lstlisting is a hard LaTeX error, and JSON, YAML, JavaScript, TypeScript and
 * Markdown are all absent from the built-in set, so an unfiltered pass-through
 * of Markdown info strings reliably produces documents that will not compile.
 * Anything unrecognised falls back to an unhighlighted listing.
 */
public final class ListingsLanguages {
    ...
    /**
     * @return the listings language name, or {@code null} when the language is
     *         unknown and the listing should be emitted without highlighting.
     */
