 ...
 *
 * <p>These exist so that a fault is reported against the Markdown line that caused it, rather than
 * surfacing later as an obscure TeX error against a generated file the author never wrote.
 */
public final class LatexSafety {

    /**
     * Characters above Latin Extended-A that the T1 encoding plus textcomp can still set.
     * Everything else outside the supported ranges is rejected.
     */
    ...
    private static final Set<Integer> EXTRA_SAFE =
            Set.of(
                    (int) '‐', // hyphen
                    ...
                    (int) '„', // double quotes
                    (int) '†',
                    (int) '‡', // dagger, double dagger
                    (int) '•', // bullet
                    (int) '…', // ellipsis
                    (int) '‰', // per mille
                    (int) '‹',
                    (int) '›', // single guillemets
                    (int) '€', // euro
                    (int) '™' // trademark
                    );
    ...
    /**
     * LaTeX's built-in list environments nest four deep. The fifth produces "Too deeply nested" and
     * stops the run.
     */
    ...
    /**
     * @return index of the first character pdflatex cannot set, or -1 if the whole string is safe
     */
    ...
    /**
     * @param translate when true, a character with a built-in translation in {@link CharacterMap}
     *     counts as supported. Pass false for verbatim contexts, where no translation can be
     *     applied.
    ...
    public static String hintFor(int codePoint) {
        ...
        if (cp >= 0x2200 && cp <= 0x22FF) {
            return "wrap mathematical symbols in $...$ so LaTeX sets them in " + "math mode";
        }
