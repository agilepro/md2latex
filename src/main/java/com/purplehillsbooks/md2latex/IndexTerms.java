...
/**
 * The book-wide set of index terms, gathered from the {@code indexTerms:} key in
 * every chapter's front matter.
 *
 * <p>Terms are pooled across the whole book: where a term is declared does not
 * matter, every chapter is searched for every term. Matching is whole-word and
 * case-insensitive, so {@code tribe} matches {@code Tribe} but not
 * {@code tribes} or {@code redistribute}.
 *
 * <p>This object is also the single source of truth for whether indexing is
 * switched on at all. If it is empty, no {@code makeidx} package and no
 * {@code \printindex} are emitted, and the book compiles in one pdflatex pass
 * exactly as before.
 */
public final class IndexTerms {
     ...
     *
     * <p>{@code ! @ | "} are makeindex's own metacharacters (sub-entry, sort
     * key, encapsulation, quote). They cannot be reliably escaped: makeindex
     * escapes its quote char as {@code \"}, which LaTeX reads as an umlaut
     * accent, and hyperref independently splits on {@code |} to append
     * {@code |hyperpage} without understanding makeindex quoting. Rejecting
     * them outright removes the whole class of problem, at no real cost for
     * author-chosen keywords.
     */
    ...
    public record Term(String canonical, String latex, Pattern pattern,
                       Path declaredIn, int declaredLine) {
    }
    ...
    /**
     * Terms whose pattern matches anywhere in {@code text}, in declaration
     * order. Does not record usage; call {@link #markUsed} for that.
     */
    ...
    /**
     * Reads the {@code indexTerms:} front matter key from every chapter and builds
     * the pooled term set.
     *
     * @param chapters chapter entries in book order, so the surviving spelling
     *                 of a term is deterministic
     * @param sources  file contents, already read once by the caller
     * @param problems collector for validation faults
     */
    public static IndexTerms collect(List<Manifest.Entry> chapters,
        ...
        for (Manifest.Entry entry : chapters) {
            ...
            if (raw == null || raw.isBlank()) {
                if (looksLikeYamlList(parsed, line)) {
                    problems.add(new Problem(Problem.Severity.ERROR, file, line, 0, sourceLine,
                            "'index' must be a comma-separated list on one line, "
                            + "not a YAML list",
                            "write it as: indexTerms: tribe, eudaimonia, moral realism"));
                } else {
                    problems.add(new Problem(Problem.Severity.WARNING, file, line, 0, sourceLine,
            ...
            for (String piece : raw.split(",")) {
                ...
                if (existing != null) {
                    if (!existing.canonical().equals(term)) {
                        problems.add(new Problem(Problem.Severity.WARNING, file, line, 0,
                                sourceLine,
                                "index term '" + term + "' was already declared as '"
                                + existing.canonical() + "' in "
    ...
    private static boolean validate(String term, Path file, int line,
                                    String sourceLine, List<Problem> problems) {
        for (int i = 0; i < term.length(); i++) {
            char c = term.charAt(i);
            if (RESERVED.indexOf(c) >= 0) {
                problems.add(new Problem(Problem.Severity.ERROR, file, line, 0, sourceLine,
                        "index term '" + term + "' contains '" + c
                        + "', which makeindex reserves",
                        "index terms may not contain any of: " + RESERVED
        ...
        if (bad >= 0) {
            ...
            problems.add(new Problem(Problem.Severity.ERROR, file, line, 0, sourceLine,
                    "index term '" + term + "' contains " + LatexSafety.describe(c)
        ...
        if (!isAscii(term)) {
            problems.add(new Problem(Problem.Severity.WARNING, file, line, 0, sourceLine,
                    "index term '" + term + "' contains non-ASCII characters, so "
    ...
    /**
     * True when the line after {@code indexTerms:} starts a YAML sequence. The front
     * matter parser is flat, so that form silently yields no terms; the
     * manifest's own {@code preamble:} key does accept it, which makes the
     * mistake an easy one.
     */
    ...
    private static Pattern compile(String term) {
        ...
        return Pattern.compile("(?<!\\w)" + Pattern.quote(term) + "(?!\\w)",
