...
public final class Preamble {
    ...
    public static void beginTheBook(Manifest manifest, Writer w) throws Exception {
        ...
        w.write("}\n");
        w.write("\\author{");
        w.write(LatexEscaper.text(manifest.author()));
        w.write("}\n");
        w.write("\\date{");
        w.write(LatexEscaper.text(manifest.date()));
        w.write("}\n\n");

        w.write("\\begin{document}\n\n");
        w.write("\\frontmatter\n\n");
        w.write("% ===== This is the TITLE page =====\n");
        w.write("\\maketitle\n\n");

        w.write("\\clearpage\n");
        w.write("% ===== This is the COPYRIGHT page =====\n");
        w.write("{\nCopyright \\copyright   ");
        w.write(LatexEscaper.text(manifest.date()));
        w.write(
                """
    ...
    \\texttt{BeaStrickland@aol.com}
        ...
        if (manifest.dedication().size() > 0) {
            w.write("\\clearpage\n");
            w.write("% ===== This is the DEDICATION page =====\n");
            w.write("\\vspace{1.75in}\n");
            w.write("\\begin{center}\n");
            for (String dedLine : manifest.dedication()) {
                w.write(LatexEscaper.textLine(dedLine));
            }
            w.write("\n\\end{center}\n\n");
        }
