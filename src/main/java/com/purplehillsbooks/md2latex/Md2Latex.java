 ...
 *
 * <p>The parser is configured once and is thread safe, so a single instance can be reused across an
 * entire directory tree. Source spans are switched on so that any problem found while rendering can
 * be reported against the line of Markdown that caused it.
 */
public final class Md2Latex {
    ...
    /**
     * The LaTeX body, the metadata found in the source front matter, and whether a requested title
     * override actually landed on a heading.
     */
    public record Converted(String latex, MarkdownLoader.Result source, boolean titleApplied) {}

    /**
     * @param markdown raw file contents
     * @param sourceFile the file being converted, used to resolve images and to name the file in
     *     any problem report
     * @param outputDir directory image paths should be relative to. For a multi-file book this is
     *     the directory holding the master .tex, not the chapter subdirectory, because LaTeX
    ...
    /**
     * @param titleOverride replaces the text of the first level-1 heading, or null to keep whatever
     *     the Markdown says
     */
    ...
    /**
     * Footnote definitions may appear after the references that use them, so they are collected
